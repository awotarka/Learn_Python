...
class Customer:
    def __init__(self, id, name, age, purchase_num, purchase_sum):
        self.name, self.age, self.purchase_num, self.purchase_sum = name, age, purchase_num, purchase_sum
        self.key = id #primary_key


# __repr__ позволяет отображать данные в человекочитаемом виде, в данном случае это словарь, но такое представление не обладает свойствами словаря


    def __repr__(self):
        return "{'name':'%s','age':%s,'num':%s,'sum':%s}" % (self.name, self.age, self.purchase_num, self.purchase_sum)

# для этого отдельно создан метод, возвращающий словарь

    def d(self):
        return {'name': self.name, 'age': self.age, 'num': self.purchase_num, 'sum': self.purchase_sum}
...
custs = {lena.key:lena.d(),artem.key:artem.d(), lidia.key:lidia.d(), fedor.key:fedor.d(), alexey.key:alexey.d(), maria.key:maria.d()}
